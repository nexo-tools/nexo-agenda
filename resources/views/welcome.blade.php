@php
    $title = __('Take bookings for your business in minutes.');
    ...
    $steps = [
        ['shot' => 'shot-1', 'caption' => __('1 — Publish your booking page')],
        ['shot' => 'shot-2', 'caption' => __('2 — Your agenda fills up')],
        ['shot' => 'shot-3', 'caption' => __('3 — Read your numbers')],
    ];
    ...
    $specs = [
        // App\Http\Requests\ServiceRequest: duration_minutes min:5 max:600.
        [__('Service duration'), __('5–600 minutes')],
        // App\Services\Availability::STEP_MINUTES.
        [__('Booking grid'), __('Slots every 15 minutes')],
        // ServiceRequest: max_advance_days min:1 max:365.
        [__('Booking horizon'), __('1–365 days per service')],
        // App\Models\Booking::newManagementToken — Str::random(48), and only
        // hash('sha256', $token) reaches the database.
        [__('Manage-booking link'), __('A 48-character token; only its SHA-256 is stored — the client creates no account')],
        // App\Console\Commands\SendBookingReminders — the 24h reminder email.
        [__('Reminder'), __('Email 24 hours before the appointment')],
        // App\Http\Requests\StoreBookingRequest: name and email required, phone
        // and note nullable.
        [__('What a client shares'), __('Name and email; phone and note optional')],
    ];
...
<x-landing-layout>
    <section data-landing-section="hero">
        <div class="mx-auto max-w-4xl px-6">
            <h1 class="max-w-2xl text-4xl font-semibold tracking-tight text-ink sm:text-5xl">{{ __('Take bookings for your business in minutes.') }}</h1>
            <p class="mt-5 max-w-xl text-lg text-muted">{{ __('Your own booking page — no commissions, and your clients don\'t need an account.') }}</p>
            <div class="mt-8 flex flex-wrap gap-3">
                <a class="nexo-btn nexo-btn--primary" href="{{ route('register') }}">{{ __('Create your booking page') }}</a>
    ...
    <section data-landing-section="producto">
        <div class="mx-auto max-w-4xl px-6">
            ...
            {{-- Kept from the landing this replaces: the directory is a real part
                 of the product, so it stays — as a link at the end of the proof,
                 not as a second button competing with the hero's CTA. --}}
            <p class="mt-8">
                <a href="{{ route('directory') }}" class="inline-flex min-h-11 items-center text-sm font-medium text-link hover:underline">{{ __('Explore businesses') }}</a>
            </p>
        </div>
    </section>
