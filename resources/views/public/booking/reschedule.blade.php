...
<x-public-layout :title="__('Reschedule').' — '.$booking->business->name" :business="$booking->business" :noindex="true">
    <a href="{{ route('booking.manage', $token) }}" class="text-sm text-brand-700 hover:underline dark:text-brand-400">
        ← {{ __('Your appointment') }}
    </a>
    <h1 class="mb-1 mt-3 text-xl font-bold">{{ __('Reschedule appointment') }}</h1>
