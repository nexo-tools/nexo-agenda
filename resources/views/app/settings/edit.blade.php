<x-app-layout>
    ...
    <h1 class="text-2xl font-bold">{{ __('Business settings') }}</h1>
    ...
    <form method="POST" action="{{ route('settings.update') }}" enctype="multipart/form-data" class="mt-6 max-w-lg space-y-4">
        ...
        <x-field :label="__('Business name')" name="name" :value="$business->name" required />
        <x-select :label="__('Category')" name="category" :selected="$business->category"
                  :options="collect(config('nexo.categories'))->mapWithKeys(fn ($c) => [$c => __('nexo.categories.'.$c)])" />
        <x-field :label="__('City')" name="city" :value="$business->city" required />
        <x-field :label="__('Address (optional)')" name="address" :value="$business->address" />
        <x-field :label="__('WhatsApp (optional)')" name="whatsapp_phone" type="tel" :value="$business->whatsapp_phone" />

        <div>
            <label for="description" class="mb-1 block text-sm font-medium">{{ __('Description (optional)') }}</label>
        ...
        <fieldset class="rounded-2xl border border-line p-4">
            <legend class="px-1 text-sm font-semibold">{{ __('Your brand on the public page') }}</legend>

            <div class="flex items-center gap-3">
                <div>
                    <label for="brand_color" class="mb-1 block text-sm font-medium">{{ __('Accent color') }}</label>
                ...
                <div class="flex-1">
                    <label for="logo" class="mb-1 block text-sm font-medium">{{ __('Logo (optional, max 1 MB)') }}</label>
            ...
            @if ($business->logo_path)
                <div class="mt-3 flex items-center gap-3">
                    <img src="{{ Storage::url($business->logo_path) }}" alt="{{ __('Current logo') }}" class="h-12 w-12 rounded-lg object-contain">
        ...
        <label class="flex items-start gap-2 text-sm">
            ...
            <span>
                {{ __('Appear in the public directory') }}
                <span class="block text-xs text-muted">
                    {{ __('Your business will be findable in /explorar by category and city. Free, no commissions.') }}
                </span>
            </span>
        </label>
