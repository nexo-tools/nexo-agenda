<x-app-layout>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <nav class="flex rounded-lg bg-slate-200 p-0.5 text-sm dark:bg-slate-700" aria-label="{{ __('Período') }}">
            @foreach (['30d' => __('30 días'), 'month' => __('Este mes'), 'last_month' => __('Mes pasado')] as $key => $label)
                <a href="{{ route('stats', ['period' => $key]) }}"
                   @class(['rounded-md px-3 py-1.5', 'bg-white font-medium shadow-sm dark:bg-slate-900' => $period === $key])>
                    {{ $label }}
                </a>
            @endforeach
        </nav>
    ...
    <p class="mt-1 text-sm text-slate-500">{{ $periodLabel }} · {{ $from->isoFormat('D MMM') }} – {{ $to->isoFormat('D MMM') }}</p>
    ...
    <div class="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
        @foreach ([
            ...
            <div class="rounded-2xl bg-white p-4 shadow-sm dark:bg-slate-800">
                ...
                @if ($hint)
                    <p class="text-xs text-slate-500">{{ $hint }} {{ __('de los turnos') }}</p>
                @endif
            </div>
        @endforeach
    </div>
    ...
    <section class="mt-6 rounded-2xl bg-white p-4 shadow-sm dark:bg-slate-800" aria-label="{{ __('Turnos por día') }}">
        ...
        <details class="mt-3">
            <summary class="cursor-pointer text-xs text-slate-500">{{ __('Ver como tabla') }}</summary>
    ...
    <div class="mt-6 grid gap-4 md:grid-cols-2">
        @foreach ([[__('Servicios más reservados'), $stats['top_services']], [__('Profesionales más reservados'), $stats['top_professionals']]] as [$title, $items])
            <section class="rounded-2xl bg-white p-4 shadow-sm dark:bg-slate-800">
                ...
                @if ($items->isEmpty())
                    <p class="mt-2 text-sm text-slate-500">{{ __('Sin datos en este período.') }}</p>
                @else
