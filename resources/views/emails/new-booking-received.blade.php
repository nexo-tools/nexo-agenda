@component('emails.layout', ['businessName' => $booking->business->name])
    <h1 style="font-size:20px;margin:0 0 8px;">{{ __('Nueva reserva') }}</h1>
    <p style="margin:0;">
        {{ __(':client reservó un turno.', ['client' => $booking->client_name]) }}
    ...
    @if ($booking->note)
        <p style="font-size:14px;"><strong>{{ __('Nota') }}:</strong> {{ $booking->note }}</p>
    @endif
