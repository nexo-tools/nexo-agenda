...
| 16 | CSP strictness (2026-07-17, phase 3.1) | Self-contained CSP with `script-src 'self' 'unsafe-eval'` and `style-src 'self' 'unsafe-inline'`, not Alpine's CSP build | Alpine evaluates directive expressions at runtime (needs `unsafe-eval`) and the schedule editor uses inline `x-for`/template expressions the CSP build forbids; migrating would be a risky rewrite. No inline `<script>` exists, so scripts stay locked to same-origin and all external hosts are still blocked. |
| 17 | Public page caching (2026-07-17, phase 3.2/3.4) | Cache plain arrays of primitives, never Eloquent models | Cached Eloquent collections can unserialize into incomplete objects and 500 the page on a cache hit; primitives are store-agnostic and smaller. Availability stays live (time-sensitive). |
